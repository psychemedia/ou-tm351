...
//            if (cell.metadata.selected == true) {   // (jfb) no metadata "selected"
                var el = $('<div id="lock" class="fa fa-plus-square-o ou_activity_outer">');
                cell.element.prepend(el);
                cell.metadata.solution = "hidden";
                //TH//cell.element.css({"background-color": "#ffffff"});
    ...
    load_css('./main.css');
    var exercise_wrapper = $('<div id="dragmask" class="highlight-drag ou_activity_outer"></div>');
    ...
    for(var i in cells){
        ...
        if (found_solution == false && typeof cell.metadata.solution != "undefined") {
            if (cell.metadata.solution=="hidden") var el = $('<div id="lock" class="fa fa-plus-square-o ou_activity_outer">');
            else var el = $('<div id="lock" class="fa fa-minus-square-o ou_activity_outer">');
            cell.element.prepend(el);
            el.click( click_solution_lock);
            found_solution = true;
        }
